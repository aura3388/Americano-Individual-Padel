...
if not st.session_state["tournament_started"]:
    ...
    player_input = st.text_area(
        "Ingresa los nombres de las jugadoras (uno por línea):",
        value="\n".join(st.session_state["players"]) if st.session_state["players"] else ""
    )
    
    if st.button("Generar Fixture y Comenzar Torneo"):
        ...
        if len(names) < 4 or len(names) % 4 != 0:
            ...
            # Simulación robusta de rondas garantizando formato "Name / Name"
            # Dividir en bloques de 4 para la jornada inicial
            for r in range(len(names) // 2):
                # Rotación simple para asegurar cruces equitativos
                p = names[:]
                # Rotar elementos para generar diferentes parejas
                if r > 0:
                    p = [p[0]] + p[1:][r:] + p[1:r]
                ...
                for i in range(0, len(p), 4):
                    if i + 3 < len(p):
                        ...
                        generated_matches.append({
                            ...
                            "score1": 0,
                            "score2": 0,
                            "status": "Programado" # Programado, En juego, Finalizado
                        })
                        match_id += 1
    ...
    for idx, match in enumerate(matches):
        ...
        # Requerimiento UI: Fondo resaltado solo para "Programado", blanco/neutral para los demás
    ...
    for m in matches:
        if m["status"] == "Finalizado":
            # Extraer jugadores de las cadenas "Name / Name"
